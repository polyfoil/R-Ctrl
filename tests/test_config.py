...
def _fake_probe(monkeypatch, *, cuda: bool, vram_mb: int, name: str = "Fake GPU"):
    """Force detect_hardware down a specific branch."""
    if cuda:
...
def test_large_gpu_selects_large_v3(monkeypatch):
    _fake_probe(monkeypatch, cuda=True, vram_mb=12000)
...
def test_no_gpu_falls_back_to_cpu_int8(monkeypatch):
    _fake_probe(monkeypatch, cuda=False, vram_mb=0)
    monkeypatch.setitem(__import__("sys").modules, "ctranslate2", None)
    hw = cfg.detect_hardware()
